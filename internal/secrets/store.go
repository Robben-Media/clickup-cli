...
const (
	apiKeyKey          = "api_key"
	keyringPasswordEnv = "CLICKUP_CLI_KEYRING_PASS" //nolint:gosec // env var name, not a credential
	keyringBackendEnv  = "CLICKUP_CLI_KEYRING_BACKEND"
...
func openKeyringWithTimeout(cfg keyring.Config, timeout time.Duration) (keyring.Keyring, error) {
	...
	case <-time.After(timeout):
		return nil, fmt.Errorf("%w after %v (D-Bus SecretService may be unresponsive); "+
			"set CLICKUP_CLI_KEYRING_BACKEND=file and CLICKUP_CLI_KEYRING_PASS=<password> to use encrypted file storage instead",
