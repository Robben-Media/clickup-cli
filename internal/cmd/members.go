...
type MembersCmd struct {
	...
	ListMembers MembersListMembersCmd `cmd:"" name:"list-members" help:"List members with access to a list"`
	TaskMembers MembersTaskMembersCmd `cmd:"" name:"task-members" help:"List members involved with a task"`
}
...
func (cmd *MembersListMembersCmd) Run(ctx context.Context) error {
	...
	result, err := client.Members().ListMembers(ctx, cmd.ListID)
	if err != nil {
		return err
	}
	...
	fmt.Fprintf(os.Stderr, "Found %d members with access to list\n\n", len(result.Members))
...
func (cmd *MembersTaskMembersCmd) Run(ctx context.Context) error {
	...
	result, err := client.Members().TaskMembers(ctx, cmd.TaskID)
	if err != nil {
		return err
	}
	...
	fmt.Fprintf(os.Stderr, "Found %d members involved with task\n\n", len(result.Members))
