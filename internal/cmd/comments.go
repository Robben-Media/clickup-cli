...
type CommentsCmd struct {
	...
	Replies      CommentsRepliesCmd      `cmd:"" help:"List replies to a comment"`
	Reply        CommentsReplyCmd        `cmd:"" help:"Reply to a comment"`
	...
	Subtypes     CommentsSubtypesCmd     `cmd:"" help:"Get post subtypes for a comment type (v3)"`
}
...
type CommentsUpdateCmd struct {
	CommentID string `arg:"" required:"" help:"Comment ID"`
	Text      string `flag:"" help:"New comment text"`
	Resolved  bool   `flag:"" help:"Mark comment as resolved"`
	Assignee  int    `flag:"" help:"User ID to assign the comment to"`
}
...
type CommentsReplyCmd struct {
	CommentID string `arg:"" required:"" help:"Comment ID to reply to"`
	Text      string `flag:"" required:"" help:"Reply text"`
	Assignee  int    `flag:"" help:"User ID to assign"`
}
...
func (cmd *CommentsReplyCmd) Run(ctx context.Context) error {
	...
	result, err := client.Comments().Reply(ctx, cmd.CommentID, cmd.Text, cmd.Assignee)
	if err != nil {
		return err
	}
...
type CommentsAddListCmd struct {
	ListID   string `arg:"" required:"" help:"List ID"`
	Text     string `flag:"" required:"" help:"Comment text"`
	Assignee int    `flag:"" help:"User ID to assign"`
}
...
type CommentsViewCommentsCmd struct {
	...
	Start   int    `flag:"" help:"Pagination start offset"`
	StartID string `flag:"" help:"Pagination start ID"`
}
...
func (cmd *CommentsViewCommentsCmd) Run(ctx context.Context) error {
	...
	params := clickup.ViewCommentsParams{
		Start:   cmd.Start,
		StartID: cmd.StartID,
	}

	result, err := client.Comments().ViewComments(ctx, cmd.ViewID, params)
	if err != nil {
		return err
	}
...
type CommentsSubtypesCmd struct {
	TypeID string `arg:"" required:"" help:"Comment type ID"`
}
