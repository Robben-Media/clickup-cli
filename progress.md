...
## Task 19 - 2026-02-23
### Completed: Create Users Service

**What was implemented:**
- Created `UsersService` with Get, Invite, Update, Remove methods
- Implemented `GET /v2/team/{team_id}/user/{user_id}` (get user details)
- Implemented `POST /v2/team/{team_id}/user` (invite user to workspace)
- Implemented `PUT /v2/team/{team_id}/user/{user_id}` (update user role)
- Implemented `DELETE /v2/team/{team_id}/user/{user_id}` (remove user from workspace)
- Added UserDetail types: UserDetail, UserRef, UserResponse, InviteUserRequest, EditUserRequest
- Added CLI commands: `users get`, `users invite`, `users update`, `users remove`
- Added 8 comprehensive tests for all service methods
- Remove warns user unless --force is used

**Files changed:**
- `internal/clickup/client.go` — added Users() accessor and UsersService with all methods
- `internal/clickup/types.go` — added all UserDetail types
- `internal/clickup/client_test.go` — added 8 tests for Users service
- `internal/cmd/users.go` — created with all CLI commands
- `internal/cmd/root.go` — added UsersCmd to CLI struct
...
- Invited user response includes `invited_by` reference
- The tagliatelle linter requires nolint comment for camelCase JSON fields from ClickUp API
- Destructive operations (remove user) should warn unless --force is used

---

## Task 20 - 2026-02-23
### Completed: Extend Members Service (List/Task Members)

**What was implemented:**
- Extended `MembersService` with ListMembers and TaskMembers methods
- Implemented `GET /v2/list/{list_id}/member` (get list members)
- Implemented `GET /v2/task/{task_id}/member` (get task members)
- Added MemberUser and MemberUsersResponse types (flat user objects, not wrapped like workspace members)
- Added CLI commands: `members list-members --list <list_id>`, `members task-members --task <task_id>`
- Added 4 comprehensive tests for all new service methods

**Files changed:**
- `internal/clickup/client.go` — extended MembersService with ListMembers, TaskMembers methods
- `internal/clickup/types.go` — added MemberUser and MemberUsersResponse types
- `internal/clickup/client_test.go` — added 4 tests for new methods
- `internal/cmd/members.go` — added list-members and task-members commands
...
**Learnings:**
- List/Task members return flat user objects `{"id", "username", "email"}`, NOT wrapped in `{"user": {}}`
- The existing workspace members endpoint wraps in `{"user": {...}}` structure
- Tests requiring validation only (no HTTP call) still need mock server - can't pass nil to newTestClient

---

## Task 21 - 2026-02-23
### Completed: Create Attachments Service

**What was implemented:**
- Created `AttachmentsService` with Upload, List, Create methods
- Implemented `POST /v2/task/{task_id}/attachment` (v2 upload via multipart)
- Implemented `GET /v3/workspaces/{wid}/{parent_type}/{parent_id}/attachments` (v3 list)
- Implemented `POST /v3/workspaces/{wid}/{parent_type}/{parent_id}/attachments` (v3 upload)
- Added Attachment and AttachmentsResponse types
- Added CLI commands: `attachments upload --task <id> <file>`, `attachments list --type <type> --id <id>`, `attachments create --type <type> --id <id> <file>`
- Added 9 comprehensive tests for all service methods
- Added static error variables: errFilePathRequired, errParentTypeRequired, errParentIDRequired

**Files changed:**
- `internal/clickup/client.go` — added Attachments() accessor, error variables, and AttachmentsService with all methods
- `internal/clickup/types.go` — added Attachment and AttachmentsResponse types
- `internal/clickup/client_test.go` — added 9 tests for Attachments service
- `internal/cmd/attachments.go` — created with all CLI commands
- `internal/cmd/root.go` — added AttachmentsCmd to CLI struct
...
**Learnings:**
- PostMultipart takes an io.Reader, not a file path - need to open file first
- v2 upload is task-specific, v3 upload works for any parent type (task, list, folder, space)
- Parent type is a path segment (not query param)
- File path from CLI argument triggers gosec G304 warning - expected behavior, use nolint comment
...
**What was implemented:**
- Created `ChatService` with 19 methods for all Chat v3 API endpoints:
  - ListChannels, GetChannel, GetFollowers, GetMembers
  - CreateChannel, CreateDM, CreateLocationChannel
  - UpdateChannel, DeleteChannel
  - ListMessages, SendMessage, UpdateMessage, DeleteMessage
  - ListReactions, CreateReaction, DeleteReaction
  - ListReplies, CreateReply
  - GetTaggedUsers
- Added Chat types: ChatChannel, ChatMessage, ChatReaction, ChatPagination
- Added request/response types for all operations
- Added 11 tests for ChatService (key functionality)
- Added 20 CLI commands: chat channels, channel, channel-followers, channel-members, create-channel, create-dm, create-loc-channel, update-channel, delete-channel, messages, send, update-message, delete-message, reactions, react, unreact, replies, reply, tagged-users
- Added static error variables: errMemberRequired, errContentRequired, errReactionRequired
- All v3 endpoints use PATCH for updates (not PUT)
- Delete commands warn user unless --force is used

**Files changed:**
- `internal/clickup/client.go` — added Chat() accessor, error variables, and ChatService with all 19 methods
- `internal/clickup/types.go` — added all Chat types
- `internal/clickup/client_test.go` — added 11 tests for Chat service
- `internal/cmd/chat.go` — created with 20 CLI commands
...
**Learnings:**
- Chat is the largest v3 domain with 19 endpoints
- Cursor-based pagination with `next_page_token`
- Location channels are tied to a space/folder/list
- Message replies use same SendMessageRequest type as regular messages
- Reactions use emoji strings directly (not codes)
...
**What was implemented:**
- Created `DocsService` with 8 methods for all Docs v3 API endpoints:
  - Search, Get, GetPageListing, GetPages, GetPage
  - Create, CreatePage, EditPage
- Added Doc types: Doc, DocPage, DocPageListingItem, DocsResponse, DocPagesResponse, DocPageListingResponse
- Added request types: CreateDocRequest, CreatePageRequest, EditPageRequest
- Added 8 tests for DocsService
- Added 8 CLI commands: docs search, get, page-listing, pages, page, create, create-page, edit-page
- Edit page uses PUT (not PATCH like other v3 updates)
- Content can be markdown or HTML

**Files changed:**
- `internal/clickup/client.go` — added Docs() accessor and DocsService with all 8 methods
- `internal/clickup/types.go` — added all Doc types
- `internal/clickup/client_test.go` — added 8 tests for Docs service
- `internal/cmd/docs.go` — created with 8 CLI commands
...
**Learnings:**
- Docs API uses PUT for page edits (not PATCH like Chat API)
- Page listing returns structure (ID + title) without content
- Page content format can be markdown ("md") or HTML ("html")
